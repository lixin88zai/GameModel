...
namespace util
{
    ...
    float   MusicUtil::music_volume = 1.0f;
    ...
    void MusicUtil::initialize()
    {
        ...
        music_volume = ReaderWriter::LoadFloatFromXML(kMusicVolumeKey, 1);
    ...
    void MusicUtil::set_is_background_music_on(bool is_on)
    {
        ...
        if (is_bg_music_on_)
        {
            ...
            else
            {
                ...
                if (is_on_play_scene)
                {
                    SimpleAudioEngine::getInstance()->playBackgroundMusic(bg_music_name_.c_str(), true);
                }
            }
        }
        else
