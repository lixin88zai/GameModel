...
void MsgService::Connect(const char* szIP, unsigned short nPort)
{
...
//______________________________________________________________________________________
//发送消息，如果未曾连接则先连接再发送
void MsgService::SendMsg(Message *pMsg, unsigned int dwType, unsigned int dwSeqID/*=0*/)
{
...
void MsgService::DumpStandbyMsg(CPtrlist* pList, bool bClear/*=true*/)
{
	if (!pList) return;
	EnterCriticalSection(&m_cs);
...
void MsgService::ClearStandbyMsg()
{
	EnterCriticalSection(&m_cs);
	NODEPOS pos = m_listMsg.GetHeadPos();
	while (pos != 0)
...
//______________________________________________________________________________________
unsigned int MsgService::GetLastReceivedMsgSeqId()
{
    return m_pSocket->GetLastReceivedMsgSeqId();
}
//______________________________________________________________________________________
void MsgService::Configure()
{
    m_pSocket->Reset();
    m_pSocket->RegisterOnConnected(this, onConnHandler_selector(MsgService::OnServiceConnected));
    m_pSocket->RegisterOnClosed(this, onCloseHandler_selector(MsgService::OnServiceClosed));
}
//______________________________________________________________________________________
void MsgService::OnServiceConnected(bool bConnected)
{
	EnterCriticalSection(&m_cs);
...
//______________________________________________________________________________________
void MsgService::OnServiceClosed()
{
